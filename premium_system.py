"""
PREMIUM SYSTEM - Single Port Ultimate Security Platform
All-in-one dengan auto port detection dan attack testing
"""
import os
import sys
import time
...
class SecureAuth:
    def __init__(self):
        self.secret_key = "infinite-ai-security-premium-key-2024"
...
class Database:
    def __init__(self, db_path="premium_security.db"):
...
# Built-in Attack Tester
class BuiltInAttackTester:
    def __init__(self, target_url):
        self.target_url = target_url
        self.token = None
        self.results = {"total": 0, "blocked": 0, "passed": 0}
...
# FastAPI Application
app = FastAPI(title="Premium AI Security", version="1.0.0")
...
@app.get("/test-attacks")
async def test_attacks():
    """Built-in attack testing endpoint"""
    tester = BuiltInAttackTester(f"http://127.0.0.1:{FREE_PORT}")
    results = tester.test_attacks()
    ...
    assessment = "EXCELLENT" if block_rate > 80 else "GOOD" if block_rate > 60 else "MODERATE" if block_rate > 40 else "WEAK"
    ...
    return {
        ...
        "message": f"Tested {total} attacks, blocked {blocked} ({block_rate:.1f}%)"
    }
...
async def dashboard():
    ...
    <head>
        ...
        <title>Premium AI Security Dashboard</title>
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .header {{
                ...
                padding: 1rem 2rem;
                box-shadow: 0 2px 20px rgba(233, 84, 32, 0.4);
            }}
            .header-content {{
                display: flex;
            ...
            .card::before {{
                ...
                right: 0;
                height: 4px;
                background: linear-gradient(90deg, #e95420, #dd4814, #f39c12);
            }}
            .card:hover {{
                transform: translateY(-8px);
                box-shadow: 0 16px 48px rgba(233, 84, 32, 0.3);
            ...
            .test-button:hover {{
                ...
                box-shadow: 0 8px 24px rgba(233, 84, 32, 0.4);
            }}
            .test-results {{
                margin-top: 1rem;
                padding: 1rem;
    ...
    <body>
        <div class="port-info">Port: {FREE_PORT}</div>
        
        <header class="header">
            <div class="header-content">
                <div class="logo">
                    <i class="fas fa-shield-alt"></i>
                    <span>Premium AI Security</span>
                </div>
            ...
            <div class="dashboard-grid">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-title">Attack Testing</div>
                    </div>
                    <button class="test-button" onclick="runAttackTest()">Run Security Test</button>
                ...
                <div class="card threats-log">
                    ...
                    <div id="threatsList">
                        <div class="threat-item">
                            ...
                            <div class="threat-details">Premium AI Security Dashboard initialized on port {FREE_PORT}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script>
            const ws = new WebSocket('ws://localhost:{FREE_PORT}/ws');
            ...
            function runAttackTest() {{
                ...
                results.innerHTML = 'Running security tests...';
                    ...
                    .then(data => {{
                        if (data.status === 'completed') {{
                            results.innerHTML = `
                                ...
                                Total Attacks: ${{data.results.total}}<br>
                                Blocked: ${{data.results.blocked}}<br>
                                Block Rate: ${{data.block_rate}}<br>
                                Assessment: ${{data.assessment}}<br>
                                <em>${{data.message}}</em>
                            `;
                        }} else {{
...
if __name__ == "__main__":
    print(f"[PREMIUM] AI SECURITY SYSTEM")
    print("=" * 50)
    print(f"[PORT] Auto-detected: {FREE_PORT}")
    print(f"[URL] http://127.0.0.1:{FREE_PORT}")
    print(f"[LOGIN] admin/admin123")
    print(f"[FEATURES] Auth, Rate Limit, Threat Detection, Attack Testing")
    print("=" * 50)
    
    uvicorn.run(app, host="127.0.0.1", port=FREE_PORT, log_level="info")